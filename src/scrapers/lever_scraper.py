...
def get_company_name(url):
    # e.g., https://jobs.lever.co/anthropic -> anthropic
...
def scrape_lever(url: str, mode: str):
    ...
    with sync_playwright() as p:
        ...
        while True:
            # Wait for postings to load, or proceed if empty
            try:
            ...
            for posting in postings:
                # Title
                title_loc = posting.locator("[data-qa='posting-name']")
                if title_loc.count() == 0:
                    title_loc = posting.locator("h5")
                ...
                # URL
                link_loc = posting.locator("a.posting-title")
                application_url = (
                    link_loc.first.get_attribute("href") if link_loc.count() > 0 else ""
                )

                # Location
                location_loc = posting.locator(".sort-by-location")
                location = (
            ...
            try:
                ...
                try:
                    page.wait_for_selector(
                        ".content, .section-wrapper, body", timeout=5000
                    )
                except Exception:
                    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", required=True, help="URL of the Lever company page")
